...
int main(const int argc, const char** argv) {
  ...
  // Allocate host memory (pinned)
    cudaMallocHost(&h_bodies.x, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.y, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.z, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.vx, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.vy, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.vz, nBodies * sizeof(float));
    cudaMallocHost(&h_bodies.mass, nBodies * sizeof(float));
    ...
    // Free host memory
    cudaFreeHost(h_bodies.x);
    cudaFreeHost(h_bodies.y);
    cudaFreeHost(h_bodies.z);
    cudaFreeHost(h_bodies.vx);
    cudaFreeHost(h_bodies.vy);
    cudaFreeHost(h_bodies.vz);
